...
2. Set `official` to `false` unless you are the first-party package. Please
   label unofficial work unofficial.
3. A real `preview` image URL helps a lot. A screenshot from the running UI,
   not a mock, not a generated product shot.
4. Set `category` if you can: `runtime` | `tokens` | `skin` | `companion`.
...
6. Commit the JSON, the rendered README, and `docs/themes.json` /
